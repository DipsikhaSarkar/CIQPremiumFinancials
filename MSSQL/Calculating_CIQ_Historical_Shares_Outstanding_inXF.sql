...
This query  calculates historical, company-level shares outstanding using 
the SP Capital IQ Market Data and Equity Capital Structure packages in Xpressfeed
for a particular CompanyId (Here for Apollo Global Management.Inc.)
...
SET @CoID = 18833
SET @sharesDate = '2016-03-07' -- year-month-day

SELECT so.companyId, 
SUM ( so.ConvertedTCSO ) AS TCSO 
...
ec.TCSOFlag, ( CASE WHEN ec.primaryClassForTCSO = 2 THEN 1 ELSE 0 END ) AS PrimaryFlag, 
0 AS NonTradedFlag , 
...
JOIN ciqTradingItem ti ( NOLOCK ) 
ON ti.securityId = ec.securityId 
AND ti.primaryFlag = 1 
JOIN ciqPriceEquitySharesOutstdg so ( NOLOCK ) 
ON so.tradingItemId = ti.tradingItemId
...
AND ec.TCSOFlag = 2 --the equity component is included in total shares outstanding as it participates in earnings and is part of EPS shares
...
ec.capitalStructureDescription AS SecurityName, 
ec.TCSOFlag, ( CASE WHEN ec.primaryClassForTCSO = 2 THEN 1 ELSE 0 END ) AS PrimaryFlag, 1 AS NonTradedFlag, 
ec.classConversionFactorForTCSO AS TSCOConvFactor, 
...
FROM ciqCapStEqComponent ec ( NOLOCK ) 
JOIN ciqNonTradedSharesOutstding ntso ( NOLOCK ) 
ON ntso.componentId = ec.componentId
